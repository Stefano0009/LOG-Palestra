(() => {
  ...
  const EXERCISE_GROUPS = {
    "Gambe": ["Stacco", "Squat con bilanciere", "Affondi", "Leg extension", "Pressa"],
    "Dorso": ["Trazioni", "Pulley", "Lat machine (presa stretta)", "Rowing machine"],
    "Petto": ["Panca", "Piegamenti declinati", "Piegamenti a terra", "Croci manubri"],
  ...
  function renderExercises(day) {
    ...
    exerciseList.innerHTML = day.exercises.map(ex => renderExerciseCard(ex)).join("");
    ...
    return `
      <div class="exercise-card" data-ex-id="${ex.id}">
        <div class="exercise-head">
          ...
          <button class="remove-ex" title="Rimuovi esercizio">${ICON_TRASH}</button>
        </div>
  ...
  function bindExerciseCardEvents(day) {
    exerciseList.querySelectorAll(".exercise-card").forEach(card => {
      ...
      card.querySelector(".remove-ex").addEventListener("click", () => {
